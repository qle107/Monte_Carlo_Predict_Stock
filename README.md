...
> Self-hosted stock dashboard: regime signal, Monte Carlo paths, scanner, backtest. One Python process, no npm build.

**Paper trading / research only. Not advice.**
...
cp .env.example .env
# Edit .env - set API keys (FRED, Alpaca, etc.) as needed
...
Default ticker is `DEFAULT_TICKER` in `config.py` (currently `PLTR`). Change it there, or pick a symbol in the dashboard / `POST /api/config`.
...
| `sentiment.py` | `/api/sentiment`, `/ws/news` | Unusual activity detector added in 
...
All tunables live in `config.py` as a `Config` dataclass. Most fields can be overridden via `.env` or `POST /api/config` at runtime - no restart required. Startup ticker comes from `DEFAULT_TICKER` in `config.py` (not `.env`).

| Variable | Default | Notes |
|---|---|---|
| `DEFAULT_TICKER` (`config.py`) | `PLTR` | Startup symbol; change in code or via dashboard / `POST /api/config` |
...
> `HMM_ENABLED` / `HAWKES_ENABLED` only affect the poll loop. `/api/market-structure` always runs both.
