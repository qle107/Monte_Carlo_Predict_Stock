...
class Config:
    ...
    # Backtest costs
    backtest_band_pct: float = field(
        default_factory=lambda: _env_float("BACKTEST_BAND_PCT", 0.003, 0.0, 0.05)
    )
    backtest_commission: float = field(
        default_factory=lambda: _env_float("BACKTEST_COMMISSION", 0.001, 0.0, 0.05)
    )
    backtest_slippage: float = field(
        default_factory=lambda: _env_float("BACKTEST_SLIPPAGE", 0.0005, 0.0, 0.02)
    )
