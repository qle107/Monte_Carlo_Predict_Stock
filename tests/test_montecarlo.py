...
_VALID_MODELS = ("gaussian", "student_t", "garch", "bootstrap", "jump",
                 "ensemble", "microstructure")
...
def _mc(df, model="garch", n_sim=300, n_fwd=10, with_microstructure_inputs=False):
    ...
    return run_mc(price, sig, n_simulations=n_sim, n_candles=n_fwd,
                  ...
                  kurtosis_excess=ind.kurtosis, **extras), price
...
def test_microstructure_with_full_inputs_produces_valid_paths(synth_df):
    ...
    volume-scaled σ.
    """
    mc, entry = _mc(synth_df, model="microstructure",
                    with_microstructure_inputs=True, n_sim=400)
...
def test_microstructure_falls_back_without_microstructure_inputs(synth_df):
    ...
    """
    mc, _ = _mc(synth_df, model="microstructure",
                with_microstructure_inputs=False, n_sim=300)
    assert mc.model == "microstructure"
...
def test_microstructure_uptrend_drift_pulls_median_up(trend_up_df):
    ...
    mc, entry = _mc(trend_up_df, model="microstructure",
                    with_microstructure_inputs=True, n_sim=1000)
...
def test_ms_diagnostics_populated_for_microstructure(synth_df):
    ...
    mc, _ = _mc(synth_df, model="microstructure",
                with_microstructure_inputs=True, n_sim=300)
...
def test_ms_diagnostics_present_in_fallback_mode(synth_df):
    ...
    mc, _ = _mc(synth_df, model="microstructure",
                with_microstructure_inputs=False, n_sim=200)
    assert mc.ms_regime in ("trending", "mean-reverting", "neutral")
...
def test_microstructure_perf_n_sim_2000(synth_df):
    ...
    mc, _ = _mc(synth_df, model="microstructure",
                with_microstructure_inputs=True, n_sim=2000, n_fwd=10)
