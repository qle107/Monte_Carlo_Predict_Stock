...
def test_find_pivots_detects_obvious_lows():
    ...
    _ph, pl = _find_pivots(highs, lows, wing=2)
...
def test_remove_broken_filters_intraday_violations():
    ...
    """
    z = Zone(
        level=95.0, low=94.7, high=95.3, strength=0.5, touches=2, fresh=True, zone_type="demand", bar_idx=0
    )
...
def test_zone_result_contract_preserved(synth_df):
    ...
    for field in ("demand_zones", "supply_zones", "nearest_demand", "nearest_supply", "price_context", "atr"):
