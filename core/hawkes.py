...
def _hawkes_loglik(params: np.ndarray, times: np.ndarray) -> float:
    ...
    integral = mu * T + (alpha / beta) * sum(
        1.0 - math.exp(-beta * (times[-1] - t)) for t in times
    )
...
def _fit_hawkes(event_times: np.ndarray) -> HawkesParams:
    ...
    n = len(event_times)
    if n < _MIN_EVENTS:
        return HawkesParams(mu=0.05, alpha=0.1, beta=0.5,
                            branching_ratio=0.2, n_events=n, fit_ok=False)
    ...
    try:
        ...
        for s in starts:
            res = minimize(
                _hawkes_loglik, np.array(s), args=(t_norm,),
                method="Nelder-Mead",
        ...
        return HawkesParams(mu=float(mu), alpha=float(alpha), beta=float(beta),
                            branching_ratio=float(br), n_events=n, fit_ok=True)
...
def _heuristic_params(t_norm: np.ndarray, n_events: int) -> HawkesParams:
    ...
    return HawkesParams(mu=float(mu), alpha=float(alpha), beta=float(beta),
                        ...
                        n_events=n_events, fit_ok=False)


# ─── Intensity computation ────────────────────────────────────────────────────

def _compute_lambda(params: HawkesParams, event_times: np.ndarray,
    ...
    excitation  = sum(alpha * math.exp(-beta * (query_time - t))
                      for t in past_events)
    return float(mu + excitation)


def _historical_lambda_dist(params: HawkesParams,
...
def _zone_probs(lam: float, lam_lo: float, lam_hi: float,
                zone_type: str) -> Tuple[float, float, float, str, str]:
    """
...
def analyse_hawkes(
    ...
    zones: Optional[List[dict]] = None,   # list of {"level": float, "zone_type": str}
) -> HawkesResult:
    """
    ...
    """
    import math as math   # ensure import in closure
    ...
    try:
        ...
        for z in (zones or []):
            ...
            zone_reactions.append(ZoneReaction(
                level=lv, zone_type=ztype,
                current_lambda=float(zl),
                bounce_prob=b, break_prob=br, consolidate_prob=c,
                excitement_label=lbl, explanation=expl,
            ))
...
def _fallback_result(zones, error: str = "insufficient_data") -> HawkesResult:
    ...
    for z in (zones or []):
        ...
        ztype = str(z.get("zone_type", "demand"))
        zone_reactions.append(ZoneReaction(
            level=lv, zone_type=ztype, current_lambda=0.0,
            bounce_prob=0.45, break_prob=0.30, consolidate_prob=0.25,
            excitement_label="unknown",
            explanation="Insufficient data for Hawkes fit — showing neutral priors.",
        ))
    return HawkesResult(
        params=HawkesParams(mu=0.0, alpha=0.0, beta=0.0,
                            branching_ratio=0.0, n_events=0, fit_ok=False),
        current_lambda=0.0,
        excitement_label="unknown",
...
# Need math for _compute_lambda at module level
import math
